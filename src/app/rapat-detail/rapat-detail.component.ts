...
export class RapatDetailComponent implements OnInit {
  ...
  constructor(public us: RapatService, private router: Router, public route: ActivatedRoute, private dialogService: NbDialogService) { }
  ...
  ngOnInit() {
    this.us.getDetailRapat(this.route.snapshot.params.id).subscribe(
      (data) => {
        ...
        this.detailRapat.tanggal = data['detailRapat']['waktu_start'].split(" ")[0];
  ...
  editRapat() {
    ...
    if (this.token == false) {
      ...
      this.dialogService.open(DialogVerifikasiTokenComponent)
