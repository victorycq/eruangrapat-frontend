...
export class PermohonanDetailComponent implements OnInit {
  ...
  constructor(public us:PermohonanService, private router: Router, public route: ActivatedRoute) { }
  permohonan:any = {} as any;
  filePermohonan:Array<any> = [{} as any];
  status:string = '';
  ngOnInit() {
    this.us.detailPermohonan(this.route.snapshot.params.id).subscribe(
      (data)=> {
        ...
        this.permohonan['pimpinan_rapat'] = this.permohonan['nip_pimpinan'] + " - " + this.permohonan['pimpinan_rapat']
        this.permohonan['nama_notulen'] = this.permohonan['nip_notulen'] + " - " + this.permohonan['nama_notulen']
        console.log(data);
      },
      (error) => {
        localStorage.clear();
